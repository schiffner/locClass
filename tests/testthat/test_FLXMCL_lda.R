...
test_that("FLXMCLlda: misspecified arguments", {
	...
	expect_that(fit <- flexmix(Species ~ V1, data = iris, model = FLXMCLlda(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), throws_error("object 'V1' not found"))
	expect_that(fit <- flexmix(y ~ Sepal.Length, data = iris, model = FLXMCLlda(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), throws_error("object 'y' not found"))
	# wrong class
