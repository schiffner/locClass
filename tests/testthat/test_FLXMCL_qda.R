...
test_that("FLXMCLqda: misspecified arguments", {
	...
	expect_that(fit <- flexmix(Species ~ V1, data = iris, model = FLXMCLqda(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), throws_error("object 'V1' not found"))
	expect_that(fit <- flexmix(y ~ Sepal.Length, data = iris, model = FLXMCLqda(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), throws_error("object 'y' not found"))
	# wrong class
...
test_that("FLXMCLqda throws a warning if grouping variable is numeric", {
	...
	expect_that(tr2 <- flexmix(round(Petal.Width,0) ~ Petal.Length, data = iris, model = FLXMCLqda(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), gives_warning("'grouping' was coerced to a factor"))
})
