#' @rdname FLXMCL
# @aliases FLXMCLmajorityGenerative-class
#'
...
setClass("FLXMCLmajorityGenerative", contains = "FLXMCL")



#' @title Common Components Model
#'
#' @description This is a model driver for \code{\link[flexmix]{flexmix}} from package \pkg{flexmix} implementing the Common Components Model.
#'
...
#' @param \dots Further arguments to and from other methods, especially \code{\link{majorityGenerative}}.
#'
#' @return Returns an object of class \code{FLXMCLmajorityGenerative} inheriting from \code{FLXMCL}.
#'
#' @note This method internally calls function \code{\link{majorityGenerative}}. \code{method = "ML"} is hard-coded.
#'
#' @rdname FLXMCLmajorityGenerative
#'
#' @aliases FLXMCLmajorityGenerative
#'
#' @family mixtures majority
#'
#' @import flexmix
...
FLXMCLmajorityGenerative <- function(formula = . ~ ., ...) {
	z <- new("FLXMCLmajorityGenerative", weighted = TRUE, formula = formula,
		name = "Common Components Model")
	z@defineComponent <- expression({
		predict <- function(x) {
			## returns the joint density p(x,y), not posteriors !!!
    		ng <- length(attr(y, "lev"))
			lev1 <- names(fit$prior)
			#post <- matrix(prior, byrow = TRUE, ncol = ng, nrow = nrow(x), dimnames = list(rownames(x), lev1))
			#post <- post * dmvnorm(x, fit$mean, fit$cov)
    		post <- matrix(0, ncol = ng, nrow = nrow(x), dimnames = list(rownames(x), attr(y, "lev")))
    		post[,lev1] <- sapply(lev1, function(z) fit$prior[z] * dmvnorm(x, fit$mean, fit$cov))
			return(post)
		}
		logLik <- function(x, y) {
...
## vereinfachen
    		...
    		post[,lev1] <- sapply(lev1, function(z) log(fit$prior[z]) + dmvnorm(x, fit$mean, fit$cov, log = TRUE))
#print(head(post))
    		ll <- post[cbind(rownames(post), as.character(y))]
#print(head(ll))
	    	return(ll)
		}
		new("FLXcomponent", parameters = list(prior = fit$prior, mean = fit$mean, cov = fit$cov, 
	...
	z@fit <- function(x, y, w) {
		fit <- majorityGenerative(x, factor(y, levels = attr(y, "lev")), weights = w, method = "ML", ...)
		d <- length(fit$mean)
		fit$df <- d + d*(d-1)/2
...
#' @rdname FLXMCLmajorityGenerative
# @aliases FLXgetModelmatrix,FLXMCLmajorityGenerative-method
#'
#' @family mixtures majority
#'
#' @import flexmix
...
setMethod("FLXgetModelmatrix", signature(model = "FLXMCLmajorityGenerative"), 
