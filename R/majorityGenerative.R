...
#'
#' @description generative majority classifier
#'
#' @details The formulas for the weighted estimates of the mean, the covariance matrix and the class priors are as follows:
#'
#' Normalized weights:
...
#'
#' @aliases majorityGenerative majorityGenerative.data.frame majorityGenerative.default majorityGenerative.formula majorityGenerative.matrix
#'
#' @export
#'
#'
...
#    center <- t(matrix(sapply(lev1, function(z) colSums(xwt[g == z, , drop = FALSE])), ncol = ng, dimnames = list(colnames(x), lev1)))/as.numeric(class.weights)
    ...
    cl <- match.call()
    cl[[1L]] <- as.name("majorityGenerative")
    return(structure(list(prior = prior, counts = counts, mean = center, 
        cov = cov, lev = lev, N = n, weights = weights, method = method, call = cl), class = "majorityGenerative"))
}
...
#'     [PimaIndiansDiabetes$diabetes])
#' 
#' fit <- wmajorityGenerative(diabetes ~ ., data = PimaIndiansDiabetes, weights = ws, 
#'     subset = train)
#' pred <- predict(fit, newdata = PimaIndiansDiabetes[-train,])
#' mean(pred$class != PimaIndiansDiabetes$diabetes[-train])
