...
SEXP predosqda(SEXP s_test, SEXP s_learn, SEXP s_grouping, SEXP s_wf, SEXP s_bw, SEXP s_k, SEXP s_method, SEXP s_env)
{
	...
	int K_present = 0;							// number of classes present in weighted training data set
	int m_present = 0;							// index of present class
	...
	for(n = 0; n < N_test; n++) {
		...
		if (nas > 0) { // NAs in n-th test observation
			...
			// only one class with positive weights?
			for (m = 0; m < K; m++) {
			...
			if (K_present == 1) { // only one class in weighted training data
				// set posterior for this class to log(1) and all others to log(0)
				for (m = 0; m < K; m++) {
					posterior[n + N_test * m] = R_NegInf;				
				}
				posterior[n + N_test * m_present] = 0;
			} else { // proceed
